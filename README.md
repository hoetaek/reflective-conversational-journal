...
| **🤔 나를 위한 질문**   | 스스로 질문 생성               | 제공된 템플릿 활용             | 일반적 질문 위주               | **개인 맥락 기반 맞춤 질문**            |
...
| **📜 시간 여행 요약**   | 수동으로 정리해야 함           | 기간별 필터링 가능             | 대화 기록 요약 가능            | **자동 월간/연간 성장 보고서**          |
| **🔐 데이터 주권**      | ✅ 완전한 개인 소유            | 클라우드 저장 (자동 백업)      | 서비스 정책에 따라 다름        | **로컬 저장 (완전한 프라이버시)**       |
| **⚡ 즉시 시작**        | 펜과 종이만 있으면 시작        | 앱 설치 후 빈 화면부터         | 즉시 대화 시작 가능            | **이전 대화 맥락을 이어받아 자연스럽게** |
...
| **인사이트 발견** | 감정 분석 + 반복 주제 인식   | 개인화된 분석 리포트         | AI 패턴 검색 및 분석         | AI 텍스트 분석 기반 인사이트            |
| **자동 요약**     | 자동 요약 + 주제 하이라이트  | 주간 진행 리포트             | 월간 리뷰                    | 주간 요약 문서 생성                     |
| **주간 리뷰**     | 이메일 주간 리뷰             | 주간 진행 하이라이트         | 월간 리뷰 (주간 불확실)      | 주간 회고 문서 자동 생성                |
| **음성 지원**     | 음성 텍스트 변환             | 음성/통화 모드 (프리미엄)    | 음성-텍스트 기술             | OS 음성 인식 활용 (설명서 안내)         |
| **데이터 소유권** | 클라우드 저장 (자동 백업)    | 클라우드 저장 (동기화)       | 클라우드 저장 (접근성)       | 로컬 저장 (완전한 프라이버시)           |
| **개발자 친화**   | 웹/모바일 앱               | 모바일 앱 중심               | 웹/모바일 앱               | IDE 통합 (Obsidian vault)               |
| **커스터마이징**  | 제한된 템플릿 수정           | 커스터마이징 불가            | 커스터마이징 불가            | 명령어/템플릿 완전 수정 가능            |
| **타겟 사용자**   | 자기계발/생산성 향상가       | 정서적 웰빙 추구자           | 구조화된 가이드 선호자       | 개발자, 연구자, Obsidian 사용자         |
...
- 개발자/연구자로서 명령어나 템플릿 커스터마이징이 중요할 때
- 무료로 AI 저널링을 시작하고 싶을 때
