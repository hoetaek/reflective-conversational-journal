...
단순한 일기 앱이 아닌, **'쓰기' 없이 '대화'만으로** 성찰이 이루어지는 새로운 저널링 패러다임입니다. 로저스의 공감과 소크라테스의 질문으로 당신 안에 있는 답을 스스로 발견하도록 돕습니다.
...
### 핵심 역할 분담
- **사용자**: 대화에만 집중 (발산) - 생각과 감정을 자유롭게 표현
- **AI**: 기록과 구조화 담당 (수렴) - 대화 내용을 실시간 기록하고 What-So What-Now What 구조로 정리

이를 통해 **작성 부담 없이** 온전히 성찰에만 몰입할 수 있으며, 필요하면 자유 노트 섹션으로 **유연성**까지 제공합니다.

### 어떻게 작동하나요?

```
🚀 처음 사용: Claude Code 실행
   ↳ 자동으로 프로필 설정 → 첫 저널링 시작

📝 매일: Claude Code 실행 (명령어 입력 불필요!)
   ↳ AI가 프로필과 어제 저널 기억하며 자동으로 대화 시작
   ↳ Free Notes (실시간 기록) → Reflections (구조화) → Long-term Insights
   ↳ 당신이 스스로 답을 발견하도록 AI가 질문으로 안내

📊 주간: /weekly 입력 → 일주일간의 패턴과 성장 인사이트 도출
```

**핵심**: 명령어를 외울 필요 없이, Claude Code만 실행하면 자동으로 저널링이 시작됩니다. 답을 주는 게 아니라, 당신 안에 있는 답을 **스스로 발견하도록** 돕습니다.
...
Reflective Conversational Journal은 **'쓰기' 없이 '대화'만으로 성찰하는 새로운 저널링 패러다임**입니다. 사용자는 대화에만 집중하고, AI가 기록과 구조화를 담당하는 역할 분담을 통해 인지 부하를 제거합니다. 클라우드 서비스가 아닌 개인 컴퓨터에서 실행되며, 로저스의 공감적 이해와 소크라테스의 날카로운 질문으로 스스로 깨달음을 찾도록 돕습니다.

**핵심 특징:**
- 🧠 **개인 맥락 기반 맞춤 질문** - AI가 모든 여정을 기억하며 연결
- 💭 **소크라테스식 질문으로 맹점 발견** - 생각의 한계 확장
...
```
1. Claude Code 실행
2. 자동으로 프로필 설정 시작 (AI와 10분 정도 대화)
...
대부분의 경우 Claude Code만 실행하면 되지만, 특별히 다음 명령어들을 직접 실행해야 할 때가 있습니다:

**프로필 수정:**
```bash
/profile
```
프로필을 수정하거나 업데이트하고 싶을 때 사용합니다.

**저널링 수동 시작:**
```bash
/journal
```
세션 중간에 새로운 저널링을 시작하고 싶을 때 사용합니다.
...
이 프로젝트는 **"철학 > 구조"** 원칙을 따릅니다. 복잡한 알고리즘 대신 핵심 원칙에 집중하며, 로저스의 공감과 소크라테스의 질문을 기반으로 한 AI 페르소나가 자연스러운 대화를 통해 성찰을 돕습니다.
