...
- 불렛 포인트 사용
- 타임스탬프는 시작과 끝을 대시로 묶음 (같은 주제에 대해)
  - 형식: `**HH:MM-HH:MM**` 주제
  - 하위 항목으로 세부 내용 기록
- 생각, 경험, 감정 있는 그대로
- 의미 있는 순간에 실시간으로 추가
...
대화 내용을 검토하고 3가지 케이스 중 하나로 진행:

---

### 케이스 A: 이미 성찰 + 더 생각해볼 주제 있음

대화 중에 What/So What/Now What이 자연스럽게 나왔고, 더 생각해볼 주제도 있을 때:
...
1. **[주제1]**
   - 간단 요약 (What + So What + Now What 포함됨)
...
이미 1, 2는 잘 이야기했고, 추가로 3, 4를 더 파고들 수도 있어.

더 생각해보고 싶은 게 있어? (번호로 말해줘, 여러 개도 괜찮고 없으면 "없어")
```

---

### 케이스 B: 이미 성찰 + 더 생각해볼 주제 없음

대화 중에 충분히 성찰했고, 억지로 만들 만한 주제가 없을 때:
...
오늘 잘 이야기했네! 더 성찰하고 싶은 게 있으면 말해줘.
```

→ 사용자가 추가 요청 없으면 Long-term Insights로 바로 이동

---

### 케이스 C: 피상적 대화 (성찰 안 함)

사실 나열만 있고 So What/Now What이 없어서, 더 파고들 여지가 있을 때:
...
어떤 걸 더 생각해보고 싶어? (번호로 말해줘, 여러 개도 괜찮고 없으면 "없어")
```
...
사용자 선택에 따라 Reflections 섹션에 What What What 형식으로 작성
   - What (필수):
