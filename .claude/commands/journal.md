...
def apply_inverse_prompting_if_requested():
    ...
    user_choice = ASK("오늘의 성과나 배움을 당신의 핵심 가치와 연결해서 더 깊이 성찰해볼까요?")
...
def find_micro_macro_connection(small_moment, bigger_picture):
    ...
    return f"오늘의 {small_moment} 같은 작은 순간이 당신의 {bigger_picture}라는 더 큰 그림과 어떻게 연결될까? 이 작은 변화가 어떤 큰 변화의 시작일 수도 있을까?"
